...
function setupHackerPanel() {
    ...
    const originalHtml = threePanel.innerHTML;
    ...
    hackerButton.addEventListener('click', function() {
        if (isMobile()) {
            ...
            document.documentElement.style.setProperty('--panel-transition', 'all 0.8s cubic-bezier(0.19, 1, 0.22, 1)');
    ...
    window.addEventListener('resize', function() {
        if (hackerContent.style.display === 'block' || lawContent.style.display === 'block') {
...
function setupTerminalForm() {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', async function(e) {
            ...
            terminalResponse.innerHTML = '<span class="blink">Transmitting message...</span>';
            ...
            try {
                ...
                // Show response
                terminalResponse.className = `terminal-message ${data.success ? 'success' : 'error'}`;
    ...
    navigationLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            if (targetSection) {
                ...
                const scrollToPosition = targetPosition - (windowHeight / 2) + (sectionHeight / 2);
                
                // For very tall sections, just align to top with some padding
                const finalPosition = sectionHeight > windowHeight 
